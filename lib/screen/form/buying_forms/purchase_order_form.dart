...
class _PurchaseOrderFormState extends State<PurchaseOrderForm> {
  ...
  Future<void> submit() async {
    ...
    data['items'] = [];
    data['taxes'] = (provider.isEditing) ? [] : context.read<UserProvider>().defaultTax;
    ...
    showLoadingDialog(context, provider.isEditing ? 'Updating ${provider.pageId}' : 'Creating Your Purchase Order');
    ...
    final res = await handleRequest(
            () async => provider.isEditing ? await provider.updatePage(data) : await server.postRequest(PURCHASE_ORDER_POST, {'data': data}), context);
    ...
    if(provider.isEditing && res == false) return;
      ...
      context.read<ModuleProvider>().pushPage(res['message']['purchase_order_name']);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: (context.read<ModuleProvider>().isEditing) ? Text("Edit Purchase Order") : Text("Create Purchase Order"),
          actions: [
            Material(
                ...
                Group(
                  child: Column(
                    children: [
                        ...
                        onPressed: () async {
                          ...
                          if (res != null) {
                            ...
                            setState(() {
                              ...
                              data['supplier_name'] = selectedSupplierData['supplier_name'];
                              data['tax_id'] = selectedSupplierData["tax_id"];
                              data['supplier_address'] = selectedSupplierData["supplier_primary_address"];
                              ...
                              data['currency'] = selectedSupplierData['currency'];

                              if (data['buying_price_list'] != selectedSupplierData['default_price_list']) {
                                data['buying_price_list'] = selectedSupplierData['default_price_list'];
                                InheritedForm.of(context).items.clear();
                                InheritedForm.of(context).data['buying_price_list'] = selectedSupplierData['default_price_list'];
                              }
                      ...
                      //         child: Text(data['supplier_name']!, style: TextStyle(fontSize: 16, color: Colors.black)),
                      //       )),
                      // if (data['supplier_name'] != null)
                      //   Padding(
                      ...
                      Row(children: [
                        Flexible(
                            child: DatePicker(
                              ...
                              onChanged: (value) => setState(() => data['transaction_date'] = value),
                            )),
                        SizedBox(width: 10),
                        Flexible(
                            child: DatePicker(
                      ...
                      //         child: Text('Tax id: ${data['tax_id']!}', style: TextStyle(fontSize: 17, color: Colors.black87)),
                      //       )),
                      // if (data['tax_id'] != null)
                      //   Padding(
                      //     padding: const EdgeInsets.only(top: 8.0, left: 2, right: 2),
                ...
                Group(
                  child: Column(
                    children: [
                      // TODO Waiting Admin review Delete or update later
                      // SizedBox(height: 4),
                      // CustomTextField('bill_no', 'Invoice No',
                      ...
                      CustomTextField('cost_center', 'Cost Center',
                          ...
                          disableValidation: true,
                          onSave: (key, value) => data[key] = value,
                          onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => costCenterScreen()))),
                      ...
                      CustomTextField('buying_price_list', 'Price List'.tr(),
                          ...
                          disableValidation: true,
                          onPressed: () async {
                            final res = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => buyingPriceListScreen()));
                            if (res != null && res.isNotEmpty) {
                              setState(() {
                                if (data['buying_price_list'] != res['name']) {
                                  InheritedForm.of(context).items.clear();
                                  InheritedForm.of(context).data['buying_price_list'] = res['name'];
                      ...
                      CustomTextField('price_list_currency', 'Price List Currency',
                      ...
                      CheckBoxWidget('ignore_pricing_rule', 'Ignore Pricing Rule',
                          initialValue: data['ignore_pricing_rule'] == 1 ? true : false, onChanged: (id, value) => setState(() => data[id] = value ? 1 : 0)),
                      CustomTextField('set_warehouse', 'Set Source Warehouse'.tr(),
                      ...
                      if(data['update_stock']!=null)
                        CheckBoxWidget('update_stock', 'Update Stock',
                            initialValue: data['update_stock'] == 1 ? true : false,
                            onChanged: (id, value) => setState(() => data[id] = value ? 1 : 0)),
