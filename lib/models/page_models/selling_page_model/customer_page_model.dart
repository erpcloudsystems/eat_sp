...
class CustomerPageModel {
  ...
  List<Map<String, String>> get card1Items {
    return [
      {
        tr("Customer Balance"): currency(data['balance']),
        tr('Disabled'): data['disabled'].toString()
      },
      {
        tr('Customer Type'): data['customer_type'] ?? tr('none'),
      ...
      {
        tr('Customer Primary Address'):
            data['customer_primary_address'] ?? tr('none')
      },
      {
        tr('Primary Address'):
      ...
      {
        tr('Customer Primary Contact'):
            data['customer_primary_contact'] ?? tr('none'),
      ...
      /*{
        tr('Credit Limit'):
            (data["credit_limits"] == null || data["credit_limits"].isEmpty)
                ? tr('none')
                : currency(data["credit_limits"][0]['credit_limit'])
      },
      {
