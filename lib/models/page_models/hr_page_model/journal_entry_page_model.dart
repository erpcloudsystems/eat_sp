...
class JournalEntryPageModel {
  ...
  List<Map<String, String>> get card2Items {
    return [
      ...
      {
        tr("Total Amount in Words"):
            data['total_amount_in_words'] ?? tr('none'),
      },
      {
  ...
  List<MapEntry<String, String>> accountCardValues(int index) {
    return [
      ...
      MapEntry(
          tr('Debit'), currency(accounts[index]['debit_in_account_currency'])),
      MapEntry(tr('Credit'),
          currency(accounts[index]['credit_in_account_currency'])),
    ];
  }
