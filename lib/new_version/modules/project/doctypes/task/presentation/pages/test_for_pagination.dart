...
class PaginationList<T> extends StatefulWidget {
  final Future<List?> Function(int pageCount) future;
...
class _PaginationListState<T> extends State<PaginationList> {
  ...
  Future<void> getItems() async {
    ...
    final res = await widget.future(pageCount);

    listCount = await widget.listCount();

    newLoadCount += res?.length ?? 0;
    ...
    if (res != null) {
      if (res.isEmpty || res.length < PAGINATION_PAGE_LENGTH) {
        _noMoreItems = true;
      }
      items.addAll(res);
    }
